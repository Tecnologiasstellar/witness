...
1. Approve D-013 and the four immutable product IDs before any App Store Connect creation.
2. Approve `Witness Atlas` subscription group naming; no-trial launch; single Support tip.
3. Approvals for any commit/push, external environment creation, signing, upload, or release.
