...
import { formatDate, orderedSources, plate, sourceMark } from "@/lib/archive.es";
...
const ALT: Record<PlateKind, (name: string) => string> = {
  plate: (name) => `Ilustración original: ${name}`,
  context: (name) => `${name} en su hábitat, ilustración original`,
  detail: (name) => `Estudio de campo: ${name}, ilustración original`,
  behavior: (name) => `${name}, observación de campo, ilustración original`,
  scale: (name) => `${name} a escala junto a una figura humana, ilustración original`,
};
...
export function StatsGrid({ stats }: { stats: NonNullable<SpeciesRecord["stats"]> }) {
  const [glyph, trend] = TREND[stats.trend] ?? ["·", "tendencia desconocida"];
  ...
  const tiles: [string, string, ReactNode?][] = [
    ["Tamaño", stats.size],
    ["Longevidad", stats.lifespan],
    ["Dieta", stats.diet],
    ["Quedan", remaining, <span key="t" aria-label={trend} className="ml-1">{glyph}</span>],
  ];
  return (
        ...
        <li key={source.id} id={`source-${i + 1}`} className="grid grid-cols-[2rem_1fr] gap-4 border-b border-hairline/40 py-5">
          ...
          <div>
            ...
            <div className="mt-1 flex flex-wrap items-center gap-x-6">
              ...
              <span className="text-[13px] text-ink-muted">Consultada el {formatDate(source.lastAccessed)}</span>
            </div>
          </div>
        </li>
      ))}
...
/** The catalog's review flags are English enums; readers get words. */
const LOCATION_REVIEW: Record<string, string> = { generalized: "generalizada" };

/** Compact record label: the museum tag under the specimen. */
export function MetaPanel({ record }: { record: SpeciesRecord }) {
  const rows: [string, string][] = [
    ["Estado", record.conservationStatus.displayName],
    ["Distribución (generalizada)", record.generalizedRange],
    ["Verificación", formatDate(record.editorial.lastFactChecked)],
    ["Revisión de ubicación", LOCATION_REVIEW[record.editorial.sensitiveLocationReview] ?? record.editorial.sensitiveLocationReview],
    ["Ilustración", record.media.depictionType],
  ];
  return (
...
export function ActionCard({ record }: { record: SpeciesRecord }) {
  ...
  return (
    <div className="border-t-2 border-ink pt-6">
      <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-sepia">El acto de esta semana · {record.commonName}</p>
      ...
      <p className="mt-3 font-display text-lg italic text-sepia">con {action.destinationOrganization}</p>
      <p className="mt-2 text-[11px] font-semibold uppercase tracking-[0.14em] text-ink-muted">
        {action.effort} · {action.geographicApplicability} · verificado el {formatDate(action.lastVerified)}
      </p>
      <div className="mt-6">
        <TextLink href={action.destinationURL} external>
          Abrir · {action.destinationOrganization}&nbsp;↗
        </TextLink>
      </div>
      <p className="mt-4 max-w-[56ch] text-[13px] leading-relaxed text-ink-muted">
        Witness registra que esta puerta se abrió. Nunca afirma el resultado.
      </p>
    </div>
  );
}
      ...
      {programs.map((program) => (
        <li key={program.id} className="border-b border-hairline/40 py-5">
          ...
          <div className="mt-1 flex flex-wrap items-center gap-x-6">
            ...
            <span className="text-[13px] text-ink-muted">Verificado el {formatDate(program.lastVerified)}</span>
          </div>
        </li>
      ))}
    </ul>
