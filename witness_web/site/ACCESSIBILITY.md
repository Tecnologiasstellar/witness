...
- The map at `/map` is a figure with no links: the list beneath it carries every region name and radius, status, and cited organization as text; hover only repeats list text, and the selected species is shown by stroke weight and dash, never by color alone.
