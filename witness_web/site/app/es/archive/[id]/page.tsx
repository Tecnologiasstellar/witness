...
import {
  ...
  StoryProse,
  Threats,
} from "@/components/record.es";
import { APP_CTA_HREF, APP_CTA_LABEL, SITE_URL, allRecords, formatDate, plate, recordById } from "@/lib/archive.es";

export function generateStaticParams() {
  return allRecords().map((record) => ({ id: record.id }));
}

export async function generateMetadata({ params }: PageProps<"/es/archive/[id]">): Promise<Metadata> {
  const { id } = await params;
  const record = recordById(id);
  if (!record) return { title: "Ficha no encontrada" };
  const hero = plate(record, "plate");
  return {
    title: `${record.commonName} · ${record.scientificName}`,
    description: `${record.hook} ${record.conservationStatus.displayName}, ${record.generalizedRange}. Una ficha de Witness con fuentes e ilustraciones originales.`,
    alternates: { canonical: `/es/archive/${record.id}`, languages: { en: `/archive/${record.id}`, es: `/es/archive/${record.id}` } },
    openGraph: {
      type: "article",
      title: `${record.commonName} · ${record.scientificName}`,
      description: record.hook,
      url: `${SITE_URL}/es/archive/${record.id}`,
      images: [{ url: hero.src, width: hero.width, height: hero.height, alt: `Ilustración original: ${record.commonName}` }],
    },
  };
}
...
export default async function RecordPage({ params }: PageProps<"/es/archive/[id]">) {
  const { id } = await params;
  const record = recordById(id);
  if (!record) notFound();
  ...
  return (
    <>
      ...
      <section className="border-b border-hairline/50 py-8 md:py-12">
        <Container>
          <Breadcrumbs trail={[{ href: "/es", label: "Witness" }, { href: "/es/archive", label: "El Archivo" }, { label: record.commonName }]} />
          <div className="mt-8 grid gap-10 md:grid-cols-12 md:gap-12">
            ...
            <div className="md:col-span-5 md:order-first md:self-center">
              <Eyebrow className="text-sepia">
                Ficha {String(number).padStart(2, "0")} de {total}
              </Eyebrow>
        ...
        <Container>
          <div className="grid gap-10 md:grid-cols-12">
            ...
            <div className="md:col-span-3 md:col-start-10 md:pt-6">
              <h2 className={label}>Dónde vive</h2>
            ...
            <div className="grid gap-12 md:grid-cols-12">
              {record.stats ? (
                <div className="md:col-span-5">
                  <h2 className={label}>Qué la amenaza</h2>
                  <div className="mt-6">
              ...
              {record.reproduction ? (
                <div className="md:col-span-6 md:col-start-7">
                  <h2 className={label}>Ciclo de vida</h2>
      ...
      <section className="py-14 md:py-20">
        <Container>
          <div className="grid gap-10 md:grid-cols-2">
            <Plate record={record} kind="detail" caption={`Estudio de campo · ${record.commonName}`} />
            <Plate record={record} kind="behavior" caption="Observación de campo" className="md:pt-16" />
          </div>
        </Container>
      </section>
        ...
        <Container>
          <div className="grid gap-10 md:grid-cols-12">
            <div className="md:col-span-3">
              <h2 className={label}>Notas de campo</h2>
              <p className="mt-4 max-w-[28ch] text-[13px] leading-relaxed text-ink-muted">
                Cada pasaje lleva el número de la fuente de la que proviene. Nada de esto está escrito de memoria.
              </p>
            </div>
      ...
      <section className="bg-paper-fresh py-14 md:py-20">
        <Container>
          <div className="grid items-center gap-10 md:grid-cols-12">
            <div className="md:col-span-5">
              <Plate record={record} kind="scale" caption="Estudio de escala · junto a una figura humana" />
            </div>
            {record.insight ? (
              <div className="md:col-span-6 md:col-start-7">
                <h2 className={label}>¿Sabías que…?</h2>
          ...
          <div className="grid gap-12 md:grid-cols-12">
            ...
            {record.programs && record.programs.length > 0 ? (
              <div className="md:col-span-5 md:col-start-8">
                <h2 className={label}>Ayuda y protección</h2>
                <div className="mt-6">
        ...
        <Container>
          <div className="grid gap-10 md:grid-cols-12">
            <div className="md:col-span-4">
              <h2 className="font-display text-[clamp(1.5rem,3vw,2.25rem)] font-semibold leading-[1.16] text-ink">Fuentes y verificación</h2>
              <div className="mt-8">
            ...
            <div className="md:col-span-7 md:col-start-6">
              ...
              <p className="mt-6 max-w-[56ch] text-[13px] leading-relaxed text-ink-muted">
                Ficha verificada por última vez el {formatDate(record.editorial.lastFactChecked)}. Las ilustraciones son originales, dibujadas para Witness; no son fotografías.
              </p>
            </div>
          </div>
        </Container>
      ...
      <section className="border-t border-hairline/50 py-14 md:py-20">
        <Container>
          <div className="grid gap-10 md:grid-cols-12">
            <div className="md:col-span-6">
              <h2 className="font-display text-2xl font-semibold text-ink">En la app, esta ficha llega un lunes.</h2>
              <p className="mt-4 max-w-[46ch] text-pretty text-[16px] leading-[1.65] text-ink-muted">
                Conoce a la especie, da testimonio una vez, lleva a cabo su acto. La ficha semanal es gratuita y seguirá siéndolo.
              </p>
            </div>
            <div className="flex flex-wrap items-center gap-x-8 gap-y-4 md:col-span-5 md:col-start-8 md:self-center">
              ...
              <TextLink href="/es/archive">Volver al archivo</TextLink>
            </div>
          </div>
        </Container>
      </section>
